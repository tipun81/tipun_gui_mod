...
function zZgetRaceDescCharRec(ref)
	...
	if desc == -1 then str = '^O' .. Infinity_FetchString(desc) .. '^-' else str = Infinity_FetchString(desc) end
	return str
end
function zZbuttonClassRace()
	if zZshowClassRaceDesc == 0 then return t("CLASS_LABEL")
	else return t("RACE_BUTTON") end
end
function zZdescClassRace()
	if zZshowClassRaceDesc == 0 then
		return '^O' .. string.upper(t("RACE_BUTTON")) .. '^-\n--------------------\n' .. zZgetRaceDescCharRec(characters[currentID].race)
	else
		return '^O' .. string.upper(t("CLASS_BUTTON")) .. '^-\n--------------------\n' .. zZgetClassStr(characters[currentID])
	end
end
--newEnd
...
function zZcheckPortraitForChange()
	zZportraitCheck = characters[currentID].portrait
	zZportraitCheck = string.upper(string.sub(zZportraitCheck, 1, string.len(zZportraitCheck) - 1))
	for i=1, #zzPortraitTable do
		local portz = string.upper(zzPortraitTable[i][2])
		--if zZportraitCheck == zzPortraitTable[i][2] then
		if string.find(portz, zZportraitCheck, 1, false) then
			currentChargenPort = i
			break
		end
	end
	zZportraitCheck = 'xxxxxxxxx'
	zzGetCurrentPortrait()
end
...
function zZsetcurrQuestList()
	...
	zZquestListComplete = 0
	for k, v in pairs(zZquestListTable) do
		if v.stateType == const.ENTRY_TYPE_INPROGRESS then
			zZquestListInProgress = zZquestListInProgress + 1
		else
			zZquestListComplete = zZquestListComplete + 1
		end
	end
end
function zzNextPrevQuest(num)
	local plus = 1
	local first, last = #zZquestListTable, 1
	local done = false
	if num ~= 1 then plus = -1; first, last = 1, #zZquestListTable end
	if zZZjournalListMode == 1 and zZquestListInProgress > 0 then done = true
	elseif zZZjournalListMode == 2 and zZquestListComplete > 0 then done = true
	elseif #zZquestListTable > 0 then done = true end
	if done then
		if currQuestList == first then
			currQuestList = last
		else
			currQuestList = currQuestList + plus
		end
	end
	return done
end
...
function zZupdateLvlPercent()
	...
	if zZlevelUpIndicators == 1 then
		...
		for i = 1, numChars do
			...
			local pb = zzGetClassMinMaxXP(1)
			local zzxp = (pb - xpFirst) * multiplier
			if zzxp < 0 then zzxp = 0 end
			if     i == 1 then zZportTooltipOne   = '\n' .. stringsAll.exp .. ' '  .. zzxp
			elseif i == 2 then zZportTooltipTwo   = '\n' .. stringsAll.exp .. ' '  .. zzxp
			elseif i == 3 then zZportTooltipThree = '\n' .. stringsAll.exp .. ' '  .. zzxp
			elseif i == 4 then zZportTooltipFour  = '\n' .. stringsAll.exp .. ' '  .. zzxp
			elseif i == 5 then zZportTooltipFive  = '\n' .. stringsAll.exp .. ' '  .. zzxp
			elseif i == 6 then zZportTooltipSix   = '\n' .. stringsAll.exp .. ' '  .. zzxp
			end
			local y = 16 + 92 * ( i - 1 )
